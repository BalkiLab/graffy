...
namespace CDLib {
    ...
    id_type bgll_find_best_community_for_node(const graph& curr_graph, const vector<id_type>& labels, const bgll_objective& book, id_type vertex) {
        ...
        id_type node_max_gain_comm_index = 0;
    ...
    double bgll_vertex_mover_optimizer(const graph& g, vector<id_type>& labels, bgll_objective& book) {
        ...
        id_type num_nodes_moved = 0;
        do {
            vector<id_type> temp_comms(g.get_num_nodes(),0);
    ...
    void bgll_recover_communities(const graph& g, const vector<id_type>& labels, vector< vector<id_type> >& hier_comms) {
        vector<node_set> last_comms, comms;
        convert_labels_to_communities(hier_comms.back(), last_comms);
        convert_labels_to_communities(labels, comms);
        hier_comms.push_back(vector<id_type > (g.get_num_nodes(), 0));
        for (id_type i = 0; i < comms.size(); i++) {
            for (node_set::iterator nit = comms[i].begin(); nit != comms[i].end(); nit++) {
                id_type last_label = hier_comms[hier_comms.size() - 2][*nit];
                for (node_set::iterator nit_j = last_comms[last_label].begin(); nit_j != last_comms[last_label].end(); nit_j++)
                    hier_comms[hier_comms.size() - 1][*nit_j] = i;
            }
        }
    }

    void bgll_collapse_nodes(const graph& orig_graph, const vector< vector<id_type> >& hier_comms, graph& g, vector<id_type>& labels) {
        g.clear();
        for (id_type i = 0; i < hier_comms[hier_comms.size() - 1]; i++)
            while (g.get_num_nodes() <= hier_comms[hier_comms.size() - 1][i]) g.add_node();
        for (id_type i = 0; i < g.get_num_nodes(); i++) {
            id_type node_comm = hier_comms[hier_comms.size() - 1][i];
            for (adjacent_edges_iterator aeit = g.out_edges_begin(i); aeit != g.out_edges_end(i); aeit++) {
                id_type neigh_comm = hier_comms[hier_comms.size() - 1][aeit->first];
                g.set_edge_weight(node_comm, neigh_comm, g.get_edge_weight(node_comm, neigh_comm) + 1);
            }
        }
    ...
    void bgll_community_detection(const graph&g, vector<id_type>& init_comms, vector< vector<id_type> >& hier_comms, bgll_objective& book) {
        ...
        while (1) {
            ...
            bgll_recover_communities(g, curr_comms, hier_comms);
            //Shrink the graph
            bgll_collapse_nodes(g, hier_comms, curr_graph, curr_comms);
    ...
    public:
        ...
        virtual double objval(const graph& curr_graph, const vector<id_type>& curr_comms) const {
            double modularity = 0;
            for (unordered_map<id_type, double>::iterator it = total_weight.begin(); it != total_weight.end(); it++) {
                double int_weight = 0;
                unordered_map<id_type, double>::iterator cit = internal_weight.find(it->first);
        ...
        virtual double compute_gain(const graph& curr_graph, const vector<id_type>& labels, id_type vertex, id_type dst_comm) const {
            unordered_map<id_type, double>::iterator it = node_link_weights.find(dst_comm);
            double gain = 0;
        ...
        virtual void attach_node(const graph& curr_graph, const vector<id_type>& labels, id_type vertex, id_type dst_comm) const {
